...
function sanitizeGoogleDriveError(error, message = undefined) {
    const sanitized = new Error(message || error?.message || 'Google Drive request failed');
...
function configureGoogleAuthClient(client, cloud_number) {
    ...
    transporter.request = function (opts, callback = undefined) {
...
async function searchFile(authClient, fileName, parentId = undefined, cloud_number = undefined) {
    const drive = google.drive({ version: 'v3', auth: authClient });
    const files = [];
...
async function downloadFile(authClient, fileId, extension) {
    ...
    try {
        ...
        if (normalizedExtension === ".tif" || normalizedExtension === ".tiff") {
            fileBuffer = await (0, sharp_1.default)(fileBuffer).png().toBuffer();
            imageType = "image/png";
        }
...
async function sendListAllFilesMetadata(cloud_number, folderId = "", book = []) {
    const auth = await authorize(cloud_number);
...
async function sendDownloadFileBuffer(fileId, cloud_number) {
    const auth = await authorize(cloud_number);
    return downloadFileBuffer(auth, fileId);
}
exports.sendDownloadFileBuffer = sendDownloadFileBuffer;
