...
class BookrecordsController {
    ...
    orWhereOcrEntitySearch(query, value, entityTypes = []) {
        ...
        if (nameField || cpfField || indexImageField) {
            queryExecute.whereHas('indeximage', queryIndex => {
                ...
                if (cpfField) {
                    ...
                    queryIndex.andWhere((subQuery) => {
                        subQuery.whereRaw("REPLACE(REPLACE(REPLACE(indeximages.cpf, '.', ''), '-', ''), ' ', '') LIKE ?", [`%${cpfDigits || cpfField}%`]);
        ...
        for (const indeximage of indeximages) {
            try {
                ...
                try {
                    ...
                    if (isManualExtract) {
                        imageResult.index_text = indexText;
                    }
                    result.images.push(imageResult);
                    if (!isManualExtract || !alreadyExtractedText) {
                        await AuditLogger_1.default.record(ctx, {
                            companiesId: authenticate.companies_id,
                            userId: authenticate.id,
