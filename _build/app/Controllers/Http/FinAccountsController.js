...
class FinAccountsController {
    async index({ auth, request, response }) {
        ...
        const querySchema = Validator_1.schema.create({
            ...
            fin_entity_List: Validator_1.schema.string.optional(),
            ...
            includePreviousOpen: Validator_1.schema.boolean.optional(),
            previousOpenStartDate: Validator_1.schema.string.optional(),
        ...
        try {
            ...
            if (body.date_start && body.date_end && body.typeDate) {
                const start = luxon_1.DateTime.fromISO(body.date_start).toUTC().startOf('day').toFormat('yyyy-MM-dd HH:mm:ss');
                const end = luxon_1.DateTime.fromISO(body.date_end).toUTC().endOf('day').toFormat('yyyy-MM-dd HH:mm:ss');
                const previousOpenStart = body.previousOpenStartDate
                    ? luxon_1.DateTime.fromISO(body.previousOpenStartDate).toUTC().startOf('day').toFormat('yyyy-MM-dd HH:mm:ss')
                    : null;
                const dateColumnMap = {
                    DATE: 'DATE',
                    DATE_DUE: 'DATE_DUE',
