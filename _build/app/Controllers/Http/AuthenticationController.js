...
class AuthenticationController {
    async login({ auth, request, response }) {
        ...
        const user = await User_1.default
            ...
            query.select('id', 'name', 'shortname', 'foldername', 'cloud', 'responsablename', 'use_device_control');
        })
        ...
        if (user.company?.use_device_control) {
            if (!device_identifier) {
                throw new BadRequestException_1.default('Dispositivo não informado', 400, 'device_error_001');
            }
            const authorizedDevice = await AuthorizedDevice_1.default
                .query()
                ...
                .first();
            if (!authorizedDevice) {
                throw new BadRequestException_1.default('Dispositivo não autorizado para esta empresa', 403, 'device_error_002');
            }
