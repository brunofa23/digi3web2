...
class OrderCertificatesController {
    ...
    normalizeDateBoundary(value, boundary) {
        ...
        let date = luxon_1.DateTime.fromISO(raw, { zone: 'America/Sao_Paulo' });
        if (!date.isValid) {
            date = luxon_1.DateTime.fromFormat(raw, 'dd/MM/yyyy', { zone: 'America/Sao_Paulo' });
        }
        if (!date.isValid)
            return raw;
        ...
        try {
            ...
            const marriedCertificate = await MarriedCertificate_1.default.updateOrCreate({ id: marriedData.id }, {
                ...
                dthrSchedule: marriedData.dthrSchedule && String(marriedData.dthrSchedule).trim() !== ''
                    ...
                    : null,
                dateMarriedReal: marriedData.dateMarriedReal && String(marriedData.dateMarriedReal).trim() !== ''
                    ? luxon_1.DateTime.fromISO(marriedData.dateMarriedReal, { zone: 'America/Sao_Paulo' })
                    : null,
    ...
    async saveBorn(bornData, companiesId, usrId, trx) {
        try {
            const registered = await this.upsertPerson(bornData.registered, companiesId, trx);
            if (!registered)
                throw new Error('Registered (registrado) é obrigatório');
            ...
            if (bornCertificateId) {
                bornCertificate = await BornCertificate_1.default.findOrFail(bornCertificateId, { client: trx });
            }
            else {
                bornCertificate = new BornCertificate_1.default();
            }
            ...
            bornCertificate.merge({
                ...
                birthDate: bornData.birthDate ? luxon_1.DateTime.fromISO(String(bornData.birthDate)) : null,
    ...
    async index({ auth, request }) {
        ...
        const dateEndOrderCertificate = this.normalizeDateBoundary(request.input('dateEndOrderCertificate'), 'end');
        ...
        const dateStartDocumentSchedule = this.normalizeDateBoundary(request.input('dateStartDocumentSchedule'), 'start');
        const dateEndDocumentSchedule = this.normalizeDateBoundary(request.input('dateEndDocumentSchedule'), 'end');
        ...
        try {
            const orderCertificate = await Database_1.default.transaction(async (trx) => {
                ...
                if (bookId === 3 && body.bornCertificate) {
                    ...
                    await Validator_1.validator.validate({
                        schema: Validator_1.schema.create({
                            registered: Validator_1.schema.object().members({
                                name: Validator_1.schema.string({ trim: true }),
                            }),
                        }),
                        data: parsedBorn,
        ...
        try {
            await Database_1.default.transaction(async (trx) => {
                ...
                if (bookId === 3 && body.bornCertificate) {
                    ...
                    const bornCertificateId = this.toNumber(body.certificateId ?? body.certificate_id) ??
