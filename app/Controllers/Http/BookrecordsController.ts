...
export default class BookrecordsController {
  ...
  public async generateOrUpdateBookrecords2(ctx: HttpContextContract) {
    ...
    if (body.book_replace && body.book_replace > 0) {
      const recordsToRename = await Bookrecord.query()
        .where("companies_id", authenticate.companies_id)
        .andWhere("typebooks_id", params.typebooks_id)
        ...
        for (const record of generatedArray) {
          if (record.id) {
            ...
            if (Object.keys(finalUpdateData).length > 0) {
              ...
              const updated = await Bookrecord.query({ client: trx })
                .where("id", record.id)
                .andWhere("companies_id", authenticate.companies_id)
                .andWhere("typebooks_id", params.typebooks_id)
                .first();
  ...
  public async maxBookRecord({ auth, request, response }: HttpContextContract) {
    ...
    let maxCod
    if (maxBook) {
      const maxSheetQuery = Bookrecord.query()
        .where('typebooks_id', typebooks_id)
        .andWhere('companies_id', authenticate.companies_id)
  ...
  public async resolveDocumentByProtocol({ auth, params, request, response }: HttpContextContract) {
    const authenticate = await auth.use('api').authenticate()
  ...
  public async visionOcrIndeximages(ctx: HttpContextContract) {
    ...
    for (const indeximage of indeximages) {
      try {
        ...
        const entities = extractOcrEntitiesFromText(indexText, {
          detectedSheet: sheet,
