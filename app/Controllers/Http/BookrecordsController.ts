...
export default class BookrecordsController {
  ...
  //   if (body.start_cod > body.end_cod) throw new BadRequestException("erro: codigo inicial maior que o final");

  //     async function modelBookNext(side: string | null, sheet: number | null) {
  //     if (!side || !body.model_book) return { side, sheet }; // Ignorar se side ou model_book não forem fornecidos
  //     if (body.model_book === "C") {
  //       side = null;
  //       sheet = 0;
  //     } else if (body.model_book === "F") {
  //       side = "F";
  //       sheet = sheet !== null ? sheet + 1 : null;
  //     } else if (body.model_book === "V") {
  //       side = "V";
  //       sheet = sheet !== null ? sheet + 1 : null;
  //     } else if (body.model_book === "FV") {
  //       side = side === "F" ? "V" : "F";
  //       sheet = sheet !== null ? sheet + 1 : null;
  //     } else if (body.model_book === "FVFV") {
  //       if (side === "V") sheet = sheet !== null ? sheet + 1 : null;
  //       side = side === "F" ? "V" : "F";
  //     } else if (body.model_book === "F-IMPAR") {
  //       sheet = sheet !== null ? sheet + 2 : null;
  //     } else if (body.model_book === "V-PAR") {
  //       sheet = sheet !== null ? sheet + 2 : null;
  //     }
  //     return { side, sheet };
  //   }
  ...
  //   try {
  //     //let model_book_state = { side: side || null, sheet: sheet || null }; // Garantir valores nulos se ausentes
  //     const result = await Bookrecord.query()
  //       .where('books_id', body.books_id)
  //       .andWhere('book', body.book)
  //       .andWhere('sheet', '>=', body.start_cod)
  //       .andWhere('sheet', '<=', body.end_cod)


  //     const arrayBookrecord = result.map(item => {
  //       const {side, sheet}=modelBookNext(item.side, item.sheet)
  //       return {
  //         id: item.id,
  //         typebooks_id: item.typebooks_id,
  ...
  //         sheet: sheet,
  //         side: side,
  //         approximate_term: item.approximate_term,
  //         indexbook: item.indexbook,
  //       }
  //     })

  //     console.log(arrayBookrecord)
  ...
  try {
    ...
    for (let cod = body.start_cod; cod <= body.end_cod; cod++) {
      ...
      if (body.by_sheet=='S') {
        // Busca pelo sheet
        baseRecord = result.find((r) => r.sheet === cod);
      } else {
        // Busca pelo cod
        baseRecord = result.find((r) => r.cod === cod);
      }
      ...
      generatedArray.push({
        ...
        cod: cod,
        book: baseRecord?.book ?? body.book,
        sheet: currentSheet,   // Sheet gerado com base no cod quando by_sheet=false
        side: currentSide,
        approximate_term: baseRecord?.approximate_term ?? body.approximate_term,
        indexbook: baseRecord?.indexbook ?? body.indexbook,
      });

      // Atualiza para o próximo sheet/side
      const next = modelBookNext(body.model_book, currentSide, currentSheet);
      currentSide = next.side;
      currentSheet = next.sheet ?? currentSheet;
    }
