...
export default class BookrecordsController {
  ...
  //   // Obtém apenas campos preenchíveis do Bookrecord
  ...
  //   try {
  //     // Atualiza o registro principal (bookrecord)
  ...
  //     // Atualiza o documento vinculado se necessário
  //     if (body.books_id === 13 && document && document.id) {
  //       // 🔹 Cria uma cópia filtrada do objeto, removendo os relacionamentos
  //       const cleanDocument = { ...document }

  //       delete cleanDocument.documenttype
  //       delete cleanDocument.documenttypebook

  //       // Alternativamente, se quiser garantir que apenas campos válidos sejam enviados:
  //       // const cleanDocument = request.only(Document.fillable)

  //       await Document.query()
  //         .where('id', cleanDocument.id)
  //         .andWhere('typebooks_id', body.typebooks_id)
  //         .andWhere('companies_id', authenticate.companies_id)
  ...
  //     await fileRename.updateFileName(body)

  //     return response.status(201).send({ success: true, id: params.id })
  //   } catch (error) {
