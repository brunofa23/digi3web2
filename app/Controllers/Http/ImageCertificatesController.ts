...
export default class ImageCertificatesController {
  ...
  public async show({ auth, params, response }: HttpContextContract) {
    ...
    const image = await ImageCertificate.query()
      .where('id', id)
    ...
    const parentFolder = await sendSearchFile(folderName, company.cloud)

    if (!parentFolder.length) {
      return response.notFound({ error: 'Pasta dos anexos não encontrada no Google Drive' })
    }

    const driveFile = await sendSearchFile(image.fileName, company.cloud, parentFolder[0].id)
    ...
    const fileDownload = await sendDownloadFile(driveFile[0].id, extension, company.cloud)
