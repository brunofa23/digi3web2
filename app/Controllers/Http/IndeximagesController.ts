...
function isDocumentProtocolFileName(fileName: any) {
...
export default class IndeximagesController {
  ...
  public async uploads(ctx: HttpContextContract) {
   ...
   let dataImagesRaw: any = request.input('dataImages')
    ...
    if (updateImage) {
      ...
      const useFileNameDocumentProtocol = images.length > 0 && images.every((image) =>
        isDocumentProtocolFileName(image?.clientName)
      )
  ...
  public async validateUploads({ auth, request, params, response }: HttpContextContract) {
    const authenticate = await auth.use('api').authenticate()
    const fileNames = request.input('fileNames', [])
    const dataImages = request.input('dataImages', {})

    await FileRename.validateFilesNameToId(
      fileNames,
      params,
      authenticate.companies_id,
      dataImages
  ...
  public async download(ctx: HttpContextContract) {
    ...
    const indexImage = await Indeximage.query()
      ...
      .andWhere('companies_id', authenticate.companies_id)
      .first()
    const fileDownload = await FileRename.downloadImage(
      fileName,
      typebook_id,
      authenticate.companies_id,
      company?.cloud,
      indexImage?.drive_file_id
    )
