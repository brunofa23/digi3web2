        ...
        if (guard === 'post' && authenticate.permission_level >= 3) {
          ...
          if (guard === 'patch' && authenticate.permission_level >= 3) {
            ...
            if (guard === 'destroy' && authenticate.permission_level >= 5) {
              ...
              if (guard === 'generateOrUpdateBookrecords' && authenticate.permission_level >= 3) {
                await next()
              }
              else
                if (guard === 'createorupdatebookrecords' && authenticate.permission_level >= 3) {
                  await next()
                }
                else
                  if (guard === 'destroyManyBookRecords' && authenticate.permission_level >= 5) {
                    await next()
                  }
                  else
                    if (guard === 'indeximagesinitial' && authenticate.permission_level >= 5) {
                      await next()
                    }
                    else {
